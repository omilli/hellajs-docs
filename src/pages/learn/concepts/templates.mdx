...
[Signals](/reference/core/signal) are automatically bound to the element thay are used in.
...
Inside components, you can create derived functions with or without using [`computed()`](/reference/core/computed).
...
- **[Computed](/reference/core/computed) values** (computed(() => ...)) - Expensive calculations that benefit from caching
...
Use the `resolve:` prefix only for HTML elements when you need to bypass automatic reactivity transformation.
...
const Counter = () => {
  ...
  return (
    <div>
      {/* Automatic reactive transformation for HTML elements */}
...
- Working with HTML elements where you need static function results
...
Nothing more than functions that return JSX elements (and ultimately HellaNodes). They support reactive data flow through signal props.
