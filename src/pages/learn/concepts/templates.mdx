...
Use `onDestroy` on elements to cleanup external resources when removed from the DOM. Reactive effects and event handlers are automatically cleaned up through the [nodeRegistry](/reference/dom/noderegistry).
...
- **Registry-Based Tracking** - Each element maintains a registry entry with cleanup functions for reactive bindings
