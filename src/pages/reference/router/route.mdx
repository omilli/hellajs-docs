...
```typescript
// ❌ May cause excessive updates - accessing route() repeatedly
effect(() => {
  console.log('Route changed:', route().path);
  console.log('Route params:', route().params);
  console.log('Route query:', route().query);
  // Multiple route() calls can cause performance issues
});
...
// ✅ Single route() access in effect
effect(() => {
  const currentRoute = route();
  console.log('Route changed:', currentRoute.path);
